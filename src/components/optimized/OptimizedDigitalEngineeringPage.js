/**
 * OptimizedDigitalEngineeringPage - Performance-optimized version using Intersection Observer
 * Demonstrates how to optimize scroll-based animations and interactions
 */
...
// Import optimized intersection observer hooks
import { 
  ...
  useScrollProgress,
  IntersectionPresets 
} from '../../animations/intersection';

const OptimizedDigitalEngineeringPage = () => {
  const [showBackToTop, setShowBackToTop] = useState(false);

  // Optimized back to top button using intersection observer
  const { ref: backToTopRef, isVisible: isBackToTopVisible } = useVisibility({
    threshold: 0.1,
    rootMargin: '300px'
  });
      ...
      <section id="main-content" className="py-16 lg:py-24 bg-gray-50">
        <Wrapper>
          <div className="grid lg:grid-cols-4 gap-12">
            ...
            <OptimizedSidebar services={allServices} />

            {/* Main Content Area */}
            <OptimizedMainContent aiServices={aiServices} />
          </div>
        </Wrapper>
      </section>
...
const OptimizedSidebar = ({ services }) => {
  const { ref, shouldAnimate } = useAnimationTrigger(IntersectionPresets.fadeIn);
      ...
      <div className="sticky top-8">
        ...
        <nav className="space-y-1">
          {services.map((service, index) => (
            <OptimizedServiceLink key={index} service={service} index={index} />
          ))}
        </nav>
      </div>
    </motion.div>
...
// Optimized Service Link Component
const OptimizedServiceLink = ({ service, index }) => {
...
const OptimizedMainContent = ({ aiServices }) => {
  const { ref, shouldAnimate } = useAnimationTrigger(IntersectionPresets.staggered);
  ...
  return (
    ...
    >
      ...
      {/* AI Services Section */}
      <OptimizedAIServicesSection aiServices={aiServices} />
...
// Optimized Introduction Section
const OptimizedIntroductionSection = () => {
  const { ref, shouldAnimate } = useAnimationTrigger(IntersectionPresets.fadeIn);
...
// Optimized AI Services Section
const OptimizedAIServicesSection = ({ aiServices }) => {
  const { ref, shouldAnimate } = useAnimationTrigger(IntersectionPresets.staggered);
...
export default OptimizedDigitalEngineeringPage;
